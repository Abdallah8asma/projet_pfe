angular
	.module('gpro.factureAvoirAchat', ["ngResource"])
	.controller(
		'factureAvoirAchatController',
		[
			'$scope',
			'$filter',
			'$http',
			...
			function ($scope, $filter, $http, $log, downloadService, UrlCommun, UrlAtelier,$window,FactureAchatServices) {
					...
					if (angular.isDefined(idClient)) {
						if (idClient != null) {
							$http
								.get(
									UrlAtelier
									+ "/factureAchat/getAvailableListFactureRefByClient:" + idClient)
								.success(
									function (resultat) {
				...
				$scope.validerFacture = function () {
					...
					if ($scope.idFactureVente == '') {
						...
						$http
							...
							.success(
								function (resultat) {
									...
									angular.forEach(resultat.listDetFactureAchat, function (element, key) {
										element.id = null;
										$scope.listDetFactureAchatPRBL.push(element);
									});
									 $scope.factureVenteCourant.idDepot = resultat.idDepot;
									//listDetFactureAchatPRBL
									// $log.debug("-- listDetFactureAchatPRBL Size : "+ $scope.listDetFactureAchatPRBL.length);
						...
						var urlValider = UrlAtelier + "/factureAchat/validateFacture?factureVenteId=" + $scope.idFactureVente;
				...
				$scope.remiseChanged = function (index) {
					if ($scope.idFactureVente == '') {
						...
						$log.info("listDetFactureAchatPRBL Remise : " + $scope.listDetFactureAchatPRBL[index].remise);

						$scope.listDetFactureAchatPRBL[index].remise = $scope.listDetFactureAchatPRBL[index].remise;
						$scope.listDetFactureAchatPRBL[index].quantite = $scope.listDetFactureAchatPRBL[index].quantite;
						$scope.listDetFactureAchatPRBL[index].prixUnitaireHT = $scope.listDetFactureAchatPRBL[index].prixUnitaireHT;
						$log.info("-- listDetFactureAchat After Remise Change : " + JSON.stringify($scope.listDetFactureAchatPRBL, null, '  '));
						...
						if ($scope.listDetFactureAchatPRBL[index].id != null) {
							$log.info("--old-- : " + JSON.stringify($scope.listDetFactureAchatPRBL[index], null, "  "));
							$scope.factureVenteCourant.listDetFactureAchat[index].remise = $scope.listDetFactureAchatPRBL[index].remise;
							$scope.factureVenteCourant.listDetFactureAchat[index].quantite = $scope.listDetFactureAchatPRBL[index].quantite;
							$scope.factureVenteCourant.listDetFactureAchat[index].prixUnitaireHT = $scope.listDetFactureAchatPRBL[index].prixUnitaireHT;

						} else {
							$log.info("--NEW-- : " + JSON.stringify($scope.listDetFactureAchatPRBL[index], null, "  "));
							$scope.factureVenteCourant.listDetFactureAchat.push($scope.listDetFactureAchatPRBL[index]);
						}

						// $scope.factureVenteCourant.listDetFactureAchat[index].remise = $scope.listDetFactureAchatPRBL[index].remise;
						$log.info("-=====- factureVenteCourant listDetFactureAchat After Remise Change : " + JSON.stringify($scope.factureVenteCourant.listDetFactureAchat, null, '  '));
					}
				...
				$scope.modifierOuCreerFacture = function () {
					...
					$http
						.get(
							UrlAtelier
							+ "/factureAchat/getFactureById:" + $scope.idFactureVente)
						.success(
							function (datagetFactureVente) {
								...
								$scope.modeValider = "actif";
								$scope.listDetFactureAchatPRBL = datagetFactureVente.listDetFactureAchat;
								/* $http
								.post(urlValider,$scope.tagReferenceBLivList)
								.success(
										function(resultat) {
								...
								$scope.listDocFactureAchat = datagetFactureVente.listDocFactureAchat;
				...
				$scope.updateFactureVente = function (factureVente) {
					...
					var urlValider = UrlAtelier + "/factureAchat/validateFacture?factureVenteId=" + factureVente.id;
					...
					$log.debug(JSON.stringify($scope.listDetFactureAchatPRBL, null, "    "));


					$log.info("======== AVANT :");
					$log.debug("factureVente.listDetFactureAchat : " + JSON.stringify(factureVente.listDetFactureAchat, null, "    "));

					factureVente.listDetFactureAchat = $scope.listDetFactureAchatPRBL;

					$log.info("======== APRES :");
					$log.debug("listDetFactureAchatPRBL : " + JSON.stringify(factureVente.listDetFactureAchat, null, "    "));


					factureVente.listTaxeFacture = $scope.listTaxeFacture;
					factureVente.listDocFactureAchat = $scope.listDocFactureAchat;
					...
					$http.post(UrlAtelier + "/factureAchat/update", factureVente)
						.success(
							function (factureVenteId) {
								...
								//getBonLivVente 
								$http.get(UrlAtelier + "/factureAchat/getFactureById:" + factureVenteId)
									.success(
										function (dataGetFactureVente) {
											//bouton Valider Off
											$scope.modeValider = "notActif";
											...
											//getTableaux
											$scope.listTaxeFacture = dataGetFactureVente.listTaxeFacture;
											$scope.listDetFactureAchat = dataGetFactureVente.listDetFactureAchat;
											factureVente.listDocFactureAchat = $scope.listDocFactureAchat;
				...
				$scope.creerFactureVente = function (factureVente) {
					...
					$http.post(UrlAtelier + "/factureAchat/create", factureVente)
						.success(
							function (factureVenteId) {
								$scope.traitementEnCours = "false";
					        	$scope.showNotif();
								...
								//Liste des TaxeLivraisonVente (pour la table Taxe) & detailsLivraisonVente ( pour la table Produit ) correspendants à ce bon de vente
								var urlValider = UrlAtelier + "/factureAchat/validateFacture?factureVenteId=" + factureVenteId;
					...
					$http
							...
							.success(
									function(dataTypeDocumentCache) {
													 ...
													 $scope.listeTypeDocumentsCache = dataTypeDocumentCache.filter(e => e.module === 'ACHAT_AVOIR');;
				...
				$scope.download = function (id, typerapport) {
					...
					$log.debug("-- id" + id);
					var url = UrlAtelier + "/reportAchat/facture?id=" + id
				...
				$scope.downloadAllFactures = function (factureVenteCourant) {
					...
					if (factureVenteCourant.partieIntId == null) {
						...
						$log.debug("=>PI  " + factureVenteCourant.partieIntId);
						url = UrlAtelier + "/reportAchat/listfacture?referenceFacture=" + factureVenteCourant.referenceFacture
							+ "&typeFacture=Avoir"
						...
						function () {
							...
							if (searchText) {
								...
								$http
									.post(
										UrlAtelier
										+ "/factureAchat/rechercheMulticritere", factureVenteCourant)
									.success(
										function (
											largeLoad) {
											data = largeLoad.list
