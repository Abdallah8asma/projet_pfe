angular
.module('gpro.factureRetourVente', [ "ngResource" ])
.controller(
		'factureRetourVenteController',
		[
		 '$scope',
		 '$filter',
		 '$http',
			...
			$scope.listReferenceBL = [];
			  ...
			  $scope.getAvailableRefBLByClientAll = function () {
					$scope.listReferenceBL = [];
							...
							.get(
									UrlAtelier
									+ "/facture/getAvailableListFactureRefByClientAll")
									.success(
											function(resultat) {
												$log.debug("----ResultatListBL "+resultat.length);
			 ...
			 $scope.validerFacture = function(){
				 ...
				 if($scope.idFactureVente == ''){
					...
					// var urlValider = UrlAtelier+ "/facture/validateFacture";
					 ...
					 var urlValider = UrlAtelier+ "/facture/validateFacture?clientId="+piId;
		 ...
		 $scope.getArticleAvoir = function(){
					 ...
					 var urlValider = UrlAtelier+ "/facture/getArticleAvoir?clientId="+piId;
				 ...
				 .get(
						 ...
						 .success(
								 function(datagetFactureVente) {
									  ...
									  $scope.factureVenteCourant.observations = datagetFactureVente.observations;
									  $scope.factureVenteCourant.nature = datagetFactureVente.nature;
				...
				$log.debug("PI  "+factureVenteCourant.partieIntId );
				 if(factureVenteCourant.partieIntId == null){
                    ...
                    url = UrlAtelier + "/reportgc/listfacture?referenceFacture=" + factureVenteCourant.referenceFacture
									 ...
									 var fileName = ' Liste Produit avec Prix Special	'		;					
									 var a = document.createElement('a');
									 document.body.appendChild(a);
									 downloadService.download(url).then(function (result) {
										var heasersFileName = result.headers(['content-disposition']).substring(17);
