...
angular
		.module('gpro.clientprix', [ "ngResource" ])
		.controller(
				'prixClientController',
				[
						'$scope',
						'$filter',
						'$http',
						...
						function($scope, $filter, $http, $log, downloadService, UrlCommun,$window) {
							...
							$scope.traitementEnCoursPrixClient = "false";	
...
//																	+JSON.stringify(dataPICache,null," "));
													...
													$scope.listePartieInteresseeCache = $filter ('filter') (dataPICache , {famillePartieInteressee : 1});
												});
							}
							...
							$scope.validerRemiseClient = function(){
								 ...
								 angular.forEach($scope.myData, function(element, key){
										...
										if(element.remise != null){
											...
											element.prixSpecial =  element.prixUnitaire*(1-element.remise/100);
										}
							...
							$scope.downloadPrixClient = function(produitCourant){
								 $scope.traitementEnCoursPrixClient = "true";
								 
								produitCourant.famillePiId=1;
								$http({
									...
									var fileName = 'Prix_Client' + formattedDate(new Date());
