		...
		.controller(
				...
				[
						...
						function($scope, $http, $filter,$rootScope, $log, downloadService, UrlAtelier,UrlCommun,$window) {
							...
							$scope.traitementEnCoursListDevis = "false";
							$scope.traitementEnCoursDevis = "false";
...
//										.copy(commande) : {};
								...
								.get(
										UrlAtelier
												+ "/commandeVente/getCurrentReferenceByType:Devis"
												)
								.success(
										function(res) {
							...
							$scope.rechercherCommandeVente = function(commandeVenteCourante){
								...
								commandeVenteCourante.type = "Devis";
								$http
								.post(
							...
							$scope.downloadAllCommandeVente = function(commandeVenteCourante) {
								...
								$scope.traitementEnCoursListDevis = "true";
...
//									            		$scope.updateProduitCommandDetails(produitCommande);
											        	...
											        	if ($scope.productFilter.length >0){
											        		...
											        		produitCommande.tauxTVA=$scope.productFilter[0].tauxTVA;
...
//													$scope.myDataCommandeVente[index].documentCommandeVentes = $scope.listeDocumentCommandeVente;
												...
												if ($scope.myDataCommandeVente[index].dateIntroduction !== null) {
													dateDevis = $scope.modifierFormatDate($scope.myDataCommandeVente[index].dateIntroduction);
												} else {
													dateDevis = null;
												}
												...
												$scope.commandeVenteCourante = Object.assign($scope.myDataCommandeVente[index], { dateIntroduction: dateDevis },{dateLivraison:dateLivraison})
						       ...
						       $scope.getSousFamilleFilter= function(sousFamilleId){
						        ...
						        $scope.clickProduit = function( idProduit, index){
						        	...
						        	if ($scope.productFilter.length >0){
						        		...
						        		$scope.listProduitCommandes[index].tauxTVA = $scope.productFilter[0].tauxTVA;
						        		 $scope.getSousFamilleFilter($scope.productFilter[0].sousFamilleId);
