...
angular
	.module('gpro.rapprochementFournisseur', [])
	.controller(
		'rapprochementFournisseurController', [
		'$scope',
		'$http',
		'$log',
		'$filter',
	...
	//generer rapport de tous les Ordre de Fabrication. mode : List
        ...
        if(typeFile == 'application/vnd.ms-excel'){
         ...
         $window.open(fileURL)
			...
			// downloadService.download(url).then(
				...
				if (conf) {
					...
					$http
						.put(
							UrlAtelier
							+ "/detailsReglementAchat/update",detailReglement)
						.success(
							function(data) {
          ...
          cellTemplate:
            `
            <button class="ms-CommandButton-button ms-CommandButton-Gpro " ng-click="updateDetailReglementAchat(row.entity)">
