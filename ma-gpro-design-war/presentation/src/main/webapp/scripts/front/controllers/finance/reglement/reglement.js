...
angular.module('gpro.reglement', []).controller('ReglementController', [
  '$scope',
  '$http',
  '$log',
  '$filter',
  ...
  function (
    ...
    $scope.rechercheReglement = function (reglementCourante) {
      $http
        .post(
          UrlAtelier + '/reglement/rechercheMulticritere',
          reglementCourante
        )
    ...
    $scope.modifierOuCreerReglement = function () {
      ...
      // get Reglement
      $http
        .get(UrlAtelier + '/reglement/getById:' + $scope.myData[index].id)
    ...
    $scope.supprimerReglement = function () {
      $http({
        method: 'DELETE',
        url: UrlAtelier + '/reglement/delete:' + $scope.myData[$scope.index].id,
      }).success(function () {
    ...
    $scope.downloadAllReglements = function (reglementCourante) {
      ...
      url =
        UrlAtelier +
        '/reportgc/reglementList?partieIntId=' +
    ...
    $scope.ValiderGroupeClient = function (groupeClientId) {
      ...
      $http
        .get(
          UrlAtelier + '/reglement/validateByGroupeClientId:' + groupeClientId
        )
        .success(function (reglement) {
          $scope.finalBLList = reglement.listLivraisonNonRegle;
          $scope.finalFacturesList = reglement.listFactureNonRegle;
